              ...
              .then(async (response) => {
                ...
                await axios.get('https://good-learn-jjrdb.ondigitalocean.app/api/cursos_alumnos')
                  .then(async (response) => {
                    ...
                    this.leer(hijo[0].id);
                    await axios.get('https://good-learn-jjrdb.ondigitalocean.app/api/notas/show?text='+this.$route.query.id)
                      .then((response) => {
                        const notas = [];
                        for(let i = 0; i < Object.keys(response.data).length; i++){
  ...
  methods: {
    leer: async function(usuarioId: any){
      axios.get('https://good-learn-jjrdb.ondigitalocean.app/api/notificaciones')
        .then(async (response) => {
          for(let i = 0; i < Object.keys(response.data).length; i++){
            if(response.data[i].tipo_id.id == 4 && response.data[i].usuario_id.id == usuarioId){
