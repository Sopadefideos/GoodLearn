<template>
  <ion-page>
    ...
    <ion-content :fullscreen="true">
      ...
      <ion-card>
        <ion-card-header>
          <ion-card-title>Aceptado</ion-card-title>
        </ion-card-header>
    ...
    <ion-card-content>
     <ion-list lines="full">
      <a
          ...
          style="text-decoration: none"
        >
        <ion-item v-if="autorizacion.estado == 1">
          <ion-label style="font-size: 70%;">{{autorizacion.usuario_id.name}} - {{autorizacion.usuario_id.email}}</ion-label>
        </ion-item>
        </a>
      </ion-list>
    </ion-card-content>
    ...
    <ion-card>
        <ion-card-header>
          <ion-card-title>Rechazado</ion-card-title>
        </ion-card-header>
    ...
    <ion-card-content>
     <ion-list lines="full">
      <a
          ...
          style="text-decoration: none"
        >
        <ion-item v-if="autorizacion.estado == 0">
          <ion-label style="font-size: 70%;">{{autorizacion.usuario_id.name}} - {{autorizacion.usuario_id.email}}</ion-label>
        </ion-item>
        </a>
      </ion-list>
...
export default defineComponent({
  ...
  created() {
    if (localStorage.session) {
      this.credentials = JSON.parse(localStorage.session);
      axios.get('https://good-learn-jjrdb.ondigitalocean.app/api/autorizaciones/show?text=' + this.$route.query.name)
        .then((response) => {
