@extends('layouts.app', ['activePage' => 'table', 'titlePage' => __('Modificar asignatura al Curso: '.$curso['name'])])
          ...
          <form method="POST" action="{{route('curso.asignatura.update', ['asignaturas_cursos' => $asignaturas_cursos['id']])}}" autocomplete="off" class="form-horizontal">
            ...
            <div class="card ">
              <div class="card-header card-header-primary" style="background: #0D2F58">
                <h4 class="card-title" style="color: #C99255 !important">{{ __('Modificar asignatura al curso') }}</h4>
                <p class="card-category">{{ __('Informacion sobre asignaturas') }}</p>
              </div>
              <div class="card-body ">
                <div class="row">
